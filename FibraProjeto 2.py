# Conexão Inteligente: Otimização da Rede de Fibra Óptica em Áreas Urbanas - Parte 1 e 2
...
class Grafo:
    ...
    # Implementação Grau dos Vértices - Projeto 2
    def grau_dos_vertices(self):
        ...
        for v in sorted(graus):
            print(f"{v} ({self.vertices[v]}): grau {graus[v]}")

    # Implementação Coloração de Vértices - Projeto 2
    def coloracao_vertices(self):
        ...
        for v in sorted(self.vertices):
            vizinhos_cores = {cores[n] for n in adj[v] if n in cores}
            cor = 0
            while cor in vizinhos_cores:
                cor += 1
            cores[v] = cor

        print("\nColoração dos vértices:")
        for v in sorted(cores):
            print(f"{v} ({self.vertices[v]}): cor {cores[v]}")
        print(f"Número total de cores usadas: {max(cores.values()) + 1}")

    # Implementação Verificar Caminho/Ciclo Hamiltoniano - Projeto 2
    def caminho_hamiltoniano(self):
        import sys

        def hamiltoniano(atual, visitado, caminho):
            nonlocal tentativas
            tentativas += 1

            if tentativas % 1000 == 0:
                print(f"\rTentativas de caminho: {tentativas}", end="")
                sys.stdout.flush()

            if len(caminho) == len(self.vertices):
                for u, v, _ in self.arestas:
                    if (u == caminho[-1] and v == caminho[0]) or (v == caminho[-1] and u == caminho[0]):
                        print(f"\nCiclo Hamiltoniano encontrado!")
                        return caminho + [caminho[0]]  # ciclo
                return caminho  # caminho simples

            for viz, _ in adj[atual]:
                if viz not in visitado:
                    visitado.add(viz)
                    result = hamiltoniano(viz, visitado, caminho + [viz])
                    if result:
                        return result
                    visitado.remove(viz)
            return None
        ...
        if not self.vertices or not self.arestas:
            print("Grafo vazio ou não carregado.")
            return
        ...
        print("Buscando caminho ou ciclo Hamiltoniano... (isso pode demorar)")
        inicio_tempo = time.time()
        caminho = hamiltoniano(inicio, visitado, [inicio])
        fim_tempo = time.time()

        print(f"\nTempo total: {fim_tempo - inicio_tempo:.2f} segundos")

        print("\nResultado:")
        if caminho:
            nomes = [self.vertices[v] for v in caminho]
            print(" -> ".join(nomes))
        else:
            print("Não foi encontrado um caminho ou ciclo Hamiltoniano.")
...
def menu():
    ...
    while True:
        ...
        print("10. Mostrar grau dos vértices")
        print("11. Aplicar coloração dos vértices")
        print("12. Verificar caminho ou ciclo Hamiltoniano")
        ...
        if op == '1':
            ...
            grafo.grau_dos_vertices()
        elif op == '11':
            grafo.coloracao_vertices()
        elif op == '12':
